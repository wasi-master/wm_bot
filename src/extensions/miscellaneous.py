...
class Miscellaneous(commands.Cog):
    ...
    async def timing(self, ctx, time=10):
        ...
        try:
            ...
            return await message.edit(embed=discord.Embed(title=f"{ctx.author}, you didn't react with a :white_check_mark:"))
        else:
            delay = round((datetime.datetime.utcnow() - message.created_at).total_seconds() - self.bot.latency, 2)
            embed = discord.Embed(
                title=f"You reacted to this message with :white_check_mark: after {delay} seconds"
            )
