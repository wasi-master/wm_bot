...
async def setup_hook():
    """Fires during setup to load extensions and connect to DB"""
    # Call the original setup_hook of WMBot to setup sessions
    await WMBot.setup_hook(bot)
    ...
    bot.unloaded_cogs_list = unloaded
...
async def on_ready():
    ...
    unloaded = getattr(bot, "unloaded_cogs_list", [])
    # If there are errors, then we send a message to the user notifying them
    if unloaded:
        owner = bot.owner or (await bot.application_info()).owner
        await owner.send(
